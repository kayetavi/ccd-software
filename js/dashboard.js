...
const { data: existingCircuit } = await supabase
  ...
  .select("id")
  .eq("loop_id", loop.id)
  .eq("circuit_name", document.getElementById("equipment").value)
  .limit(1);

if (existingCircuit && existingCircuit.length > 0) {
  alert("⚠️ Circuit already exists in this loop");
  return;
}
    ...
    /* AUTO INSPECTION (API RP 970) */
/* AUTO INSPECTION (API RP 970) */
const { error: inspError } = await supabase.rpc(
  "auto_inspection_for_circuit",
  { cid: circuit.id }
);

if (inspError) {
  console.error("Auto inspection error:", inspError);
  alert("⚠️ Inspection generation failed (damage saved)");
}


    alert(
  "✅ CCD DATA GENERATED SUCCESSFULLY\n\n" +
  "• Corrosion Loop\n" +
  "• Circuit\n" +
  "• Damage Mechanisms (API RP 970)\n" +
  "• Inspection Strategy"
);
